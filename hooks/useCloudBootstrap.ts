...
const buildCloudCompletions = (rows: Array<{ habit_id: string; user_id: string; date: string; completed_at: string }>, userIds: { me: string; mom: string }): CompletionByDate => {
  ...
  rows.forEach((row) => {
    const actor = actorForUserId(row.user_id, userIds);
    if (!actor) return;
...
const buildCloudNudges = (
  ...
  const from = actorForUserId(row.from_user, userIds);
  const to = actorForUserId(row.to_user, userIds);
...
const buildCloudFootprints = (
  ...
  const createdBy = actorForUserId(row.created_by, userIds);
  return createdBy ? [{
    id: row.id,
    pairId: row.pair_id,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Keep the last good local state when a background refresh is interrupted.
      } finally {
        loading = false;
      }
    };
